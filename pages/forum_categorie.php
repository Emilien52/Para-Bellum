<?php if(isset($_GET['id']) AND isset($_Joueur_))
{
	...
	if($_Forum_->exist($id))
	{
		...
		?><section class="layout" id="white">
			<div class="container">
				<h1 class="titre text-uppercase"><span class="far fa-comments"></span> Forum : <?=$categoried['nom'];?></h1>
			</div>
		</section>
		...
		<div class="container">
			...
			<nav aria-label="breadcrumb" role="navigation">
				<ol class="breadcrumb">
				  ...
				  <li<?php if(isset($id_sous_forum)) { echo ' class="breadcrumb-item"><a style="color: #d82c2e;" href="?page=forum_categorie&id='.$id.'">'; } else { echo ' class="breadcrumb-item active">'; } echo $categoried['nom']; if(isset($id_sous_forum)) { echo '</a>'; } ?></li>
		...
		?><nav aria-label="Page forum catégorie">
				<ul class="pagination"><?php
					...
					{
						?><li class="page-item"><a class="page-link" href="?&page=forum_categorie&id=<?php echo $id; if(isset($id_sous_forum)) echo "&id_sous_forum=$id_sous_forum"; ?>&page_topic=<?php echo $i; ?>" style="color: #d82c2e;"><?php echo $i;
						?></a></li><?php
					} ?>    
				</ul>
			</nav>
	...
	if($categoried['close'] == 0 OR ($_Joueur_['rang'] == 1 OR $_PGrades_['PermsForum']['general']['seeForumHide'] == true) AND !$_SESSION['mode'])
	{
		?>
		<hr/>
		<h4 style="text-transform: uppercase;">Poster un topic dans la catégorie <?php echo $categoried['nom']; if(isset($id_sous_forum)) { echo ' et le sous-forum ' .$sousforumd['nom']. ''; } ?></h4>
		<form action="?&action=create_topic" method="post">
			...
			<div class="form-group row">
				<div class="col-sm-offset-12 col-sm-12">
					<button type="submit" class="btn btn-parabellum" style="float: right;">Poster</button>
				</div>
			</div>
		</form>
		<?php 
