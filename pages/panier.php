<section class="layout" id="white">
		<div class="container">
			<h1 class="titre text-uppercase"><span class="fa fa-shopping-basket"></span> Mon panier</h1>
		</div>
</section>
<section class="layout" id="page">
    <div class="container">
            	...
            	else
            	{
	            	for($i = 0; $i < $nbArticles; $i++)
	            	{
	            		...
	            		<tr>
                            ...
                            <td><a href="?action=supprItemPanier&id=<?php echo htmlspecialchars($_SESSION['panier']['id'][$i]); ?>" class="btn btn-parabellum link" title="supprimer l'item du panier"><i class="fa fa-trash"></i></a></td>
		                </tr>
		               <?php
		            } 
                    if(!empty($_SESSION['panier']['reduction']))
                    {
                        echo '<tr><td>'.htmlspecialchars($_SESSION['panier']['code']).'</td><td>'.htmlspecialchars($_SESSION['panier']['reduction_titre']).'</td><td>1</td><td class="w-25 text-center">-'. $_SESSION['panier']['reduction']*100 .'%</td><td></td><td><a href="?action=retirerReduction" class="btn btn-parabellum link" title="supprimer la réduction"><i class="fa fa-trash"></i></a></td></tr>';
                    }
		        }
		        ?>
		        <tr>
        ...
        <form class="form-inline" action="?action=ajouterCode" method="POST">
            ...
            <a><button class="btn btn-lg btn-parabellum hvr-float-shadow"> Envoyer</button></a>
        </form>
        <div class="text-right">
            <a href="?action=viderPanier"><button class="btn btn-lg btn-parabellum hvr-float-shadow">Vider le panier !</button></a>
            <a href="?action=achat"><button class="btn btn-lg btn-parabellum hvr-float-shadow">Acheter !</button></a>
        </div>
    </div>
</section>