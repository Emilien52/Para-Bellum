...
</section><section class="layout" id="page"><div class="container"><br/>
...
if(isset($_GET['choix']))
{
			...
			switch($choix)
			{
				...
				case '2':
					...
					<form action="<?php echo $_Serveur_['General']['url']; ?>?&action=forum_moderation&id_topic=<?php echo $id; ?>&choix=2&confirmation=true" method="post">
						...
						<div class="form-group row">
							<div class="col-sm-10">
								<button type="submit" class="btn btn-parabellum">Supprimer ce topic :(</button>
							</div>
							<div class="col-sm-2">
				...
				case '3':
					...
					<form action="<?php echo $_Serveur_['General']['url']; ?>?&action=forum_moderation&id_topic=<?php echo $id; ?>&choix=3&confirmation=true" method="post">
						<div class="form-group row">
							...
							<div class="col-sm-10">
								<select class="c-select text-muted" name="emplacement" id="emplacement" require >
									<?php 
									...
									while($emplacementd = $emplacement->fetch())
									{
										...
										{
											?><optgroup class="text-muted" label="<?php echo $emplacementd['nom']; ?>">
											<option class="text-muted" value="<?php echo $emplacementd['id']; ?>">Déplacer dans la catégorie</option>
											<?php
											...
											{
												?><option class="text-muted" value="<?php echo $emplacementd['id']; ?>_<?php echo $sous_forumd['id']; ?>"><?php echo $sous_forumd['nom']; ?></option><?php
											}
											?></optgroup><?php
										}
										else 
										{
											?><option class="text-muted" value="<?php echo $emplacementd['id']; ?>_0"><?php echo $emplacementd['nom']; ?></option>
										<?php 
										}
									}
								?></select>
						...
						<div class="form-group row">
							<div class="col-sm-10">
								<button type="submit" class="btn btn-parabellum">Déplacer la discussion </button>
							</div>
							<div class="col-sm-2">
					<a href="index.php" class="btn btn-warning">Annuler</a>
				</div>
	...
	if($choix == 4)
		{
			...
			<form action="?&action=signalement&confirmation=true" method="post">
				...
				<div class="row">
					<div class="col-sm-10">
						<button class="btn btn-parabellum" type="submit">Signaler ! </button>
					</div>
					<div class="col-sm-2">
					<a href="index.php" class="btn btn-warning">Annuler</a>
				</div>
	...
	if($choix == 5)
	{
		...
		<form action="?&action=signalement_topic&confirmation=true" method="post">
			...
			<div class="form-group row">
				<div class="col-sm-10">
					<button type="submit" class="btn btn-parabellum">Signaler ce topic !</button>
				</div>
				<div class="col-sm-2">
					<a href="index.php" class="btn btn-warning">Annuler</a>
				</div>
