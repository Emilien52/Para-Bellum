...
if(isset($_GET['id']) AND isset($_Joueur_))
{
	...
	?><section class="layout" id="white">
		<div class="container">
			<h1 class="titre text-uppercase"><span class="far fa-comments"></span> Forum / Post: <?=$topicd['nom'];?></h1>
		</div>
	</section>
	...
	<nav aria-label="breadcrumb" role="navigation">
		<ol class="breadcrumb" style="front-size: 20px;">
			<li class="breadcrumb-item"><a href="/" style="color: #d82c2e;">Accueil</a></li>
			<li class="breadcrumb-item"><a href="?page=forum" style="color: #d82c2e;">Forum</a></li>
			<li class="breadcrumb-item"><a href="?&page=forum_categorie&id=<?php echo $topicd['id_categorie']; ?>" style="color: #d82c2e;"><?php echo $topicd['nom_categorie']; ?></a></li>
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="dropdown">
				<button class="btn btn-parabellum dropdown-toggle" type="button" id="Actions-Modération" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" style="float: right;">
				...
				if($_PGrades_['PermsForum']['moderation']['closeTopic'] == true OR $_Joueur_['rang'] == 1)
				{
					if($topicd['etat'] == 1)
					{
						?><li> <a href="?&action=forum_moderation&id_topic=<?php echo $id; ?>&choix=4" class="text-muted">Ouvrir la discussion</a></li><?php
					}
					else
					{
						?><li><a href="?&action=forum_moderation&id_topic=<?php echo $id; ?>&choix=1" class="text-muted">Fermer la discussion</a></li>
					<?php 
					}
				}
				...
				{
					?>
					<li><a href="?&action=forum_moderation&id_topic=<?php echo $id; ?>&choix=2" class="text-muted">Supprimer le topic</a></li>
					<?php 
				}
				if($_PGrades_['PermsForum']['moderation']['mooveTopic'] == true OR $_Joueur_['rang'] == 1)
				{
					?>
					<li><a href="?&action=forum_moderation&id_topic=<?php echo $id; ?>&choix=3" class="text-muted">Déplacer la discussion</a></li>
					<?php 
				}
				?>
				</ul>
	...
	<h3 style="text-transform: uppercase;">Sujet : <?php echo $topicd['nom']; ?></h3><br/>
	<div class="row">
		<div class="col-md-2">
		...
		<!-- Contenue du topic de l'auteur -->
			...
			echo $contenue;
			?></div><br/><div style="border-top: 0.5px white solid;"></div>
			<p class="text-right">Posté le <?php  echo $topicd['jour']; ?> <?php $mois = switch_date($topicd['mois']); echo $mois; ?> <?php echo $topicd['annee'];?>  <?php if($topicd['d_edition'] != NULL) { echo 'édité le '; $d_edition = explode('-', $topicd['d_edition']); echo $d_edition[2]; echo '/' .$d_edition[1]. '/' .$d_edition[0]. ''; } ?></p>
		</div>
	</div>
	<div class="row">
		<div class="col-md-2">
	<form action="?&action=signalement_topic" method="post">
		...
		<button type="submit" class="btn btn-parabellum">Signaler !</button>
	</form>
	</div>
	<?php if($_Joueur_['pseudo'] == $topicd['pseudo'] OR ($_Joueur_['rang'] == 1 OR $_PGrades_['PermsForum']['general']['editTopic'] == true) AND !$_SESSION['mode'])
	{
		?><div class="col-md-2"><form action="?page=edit_topic" method="post">
			...
			<button type="submit" class="btn btn-rounded btn-info">Editer le topic</button>
		</form>
		</div>
		<?php 
	}
		...
		<form action="?action=remove_topic" method="post">
			<input type="hidden" name="id_topic" value="<?php echo $id; ?>" />
			<a class="btn btn-round btn-warning" role="button" data-toggle="collapse" href="#topic_<?php echo $id; ?>" aria-expanded="false" aria-controls="collapseExample" >
	...
	{ ?>
		...
		<div class="row">
			...
			<div class="col-md-10"> <!-- contenue de la réponse -->
				...
				echo $answere;
				?></div>
				<br/><div style="border-top: 0.5px white solid;"></div>
				<p class="text-right"><?php echo $answerd[$i]['day']; ?> <?php $answerd[$i]['mois'] = switch_date($answerd[$i]['mois']); echo $answerd[$i]['mois']; ?> <?php echo $answerd[$i]['annee']; ?> <?php if($answerd[$i]['d_edition'] != NULL){ echo 'édité le '; $d_edition = explode('-', $answerd[$i]['d_edition']); echo '' .$d_edition[2]. '/' .$d_edition[1]. '/' .$d_edition[0]. ''; } ?> </p>
			</div>
		</div>
		<div class="row">
		<div class="col-md-2">
			...
			if(empty($test_votedata['count']) && $_Joueur_['pseudo'] != $answerd[$i]['pseudo'])
			{
				...
				<form class="form-inline" action="?&action=like" method="post">
					...
					<input type="hidden" name="id_answer" value="<?php echo $answerd[$i]['id']; ?>" />
					<button type="submit" class="btn btn-success" title="J'aime" ><i class="far fa-thumbs-up"></i></button>
				</form></div><div class="col-md-1">
				<form class="form-inline" action="?&action=like" method="post">
					<input type="hidden" name="choix" value="2" />
					<input type="hidden" name="id_answer" value="<?php echo $answerd[$i]['id']; ?>" />
					<button type="submit" class="btn btn-warning" title="Je n'aime pas"><i class="far fa-thumbs-down"></i></button>
				</form></div>
				<?php
			}
			elseif($_Joueur_['pseudo'] != $answerd[$i]['pseudo'] AND $test_votedata['count'] > 0)
			{
				...
				<form class='form-inline' action="?&action=unlike" method="post">
					<input type="hidden" name="id_answer" value="<?php echo $answerd[$i]['id']; ?>" />
					<button type="submit" class="btn btn-parabellum" title="Ne plus aimer">Retirer</button>
				</form></div>
				<?php
			}
			if($_Joueur_['pseudo'] === $answerd[$i]['pseudo'] OR ($_PGrades_['PermsForum']['moderation']['editMessage'] == true OR $_Joueur_['rang'] == 1) AND !$_SESSION['mode'])
			{
				?><div class="col-md-2"><form action="?page=edit_answer" method="post">
					<input type="hidden" name="id_answer" value="<?php echo $answerd[$i]['id']; ?>" />
					<button type="submit" class="btn btn-info">Editer ce message</button>
				</form></div>
				<?php 
			}
			...
			<div class="col-md-2">
				<form action="?action=remove_answer" method="post">
					...
					<a role="button" class="btn btn-warning" data-toggle="collapse" href="#answer_<?php echo $answerd[$i]['id']; ?>" aria-expanded="false" aria-controls="collapseExample">
	...
	<form action="?&action=post_answer" method="post">
		...
		<div class="form-group row">
			...
			<div class="col-md-12">
				<textarea class="form-control" name="contenue" id="contenue" max="10 000" min="1" rows="10" require ></textarea>
			</div>
		</div>
		<div class="form-group row">
			<div class="col-md-12 text-center">
				<button type="submit" class="btn btn-parabellum">Poster votre réponse</button>
			</div>
		</div>
		</p>
	</form>
