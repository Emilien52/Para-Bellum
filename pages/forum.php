...
<div class="info-container">
	...
	<p>Bienvenue sur le forum de <?php echo $_Serveur_['General']['name']; ?>,
	Ici vous pourrez échanger et partager avec toute la communauté du serveur !<p></div><br>
<?php if($_Joueur_['rang'] == 1 OR $_PGrades_['PermsForum']['general']['modeJoueur'] == true)
 	{
 		?>
 			<p class="text-center">
 				<a href="?action=mode_joueur" class="btn btn-warning">Passer en mode visuel <?php echo ($_SESSION['mode']) ? "Administrateur" : "Joueur"; ?></a>
 			</p>
 		<?php 
 	}
...
{ ?>
		...
		<div class="row"><div class="col-md-9"><h3 style="text-transform: uppercase;"><?php echo ucfirst($fofo[$i]['nom']); ?></h3></div><?php if(isset($_Joueur_) AND ($_PGrades_['PermsForum']['general']['deleteForum'] == true OR $_Joueur_['rang'] == 1) AND !$_SESSION['mode']){ ?><div class="col-md-3" style="text-align: right;"><a href="?action=remove_forum&id=<?php echo $fofo[$i]['id']; ?>" class="btn btn-parabellum" style="text-align: right;">Supprimer</a></div><?php } ?></div>
		<thead>
			<tr>
				<th></th>
				<th>Nom</th>
...
<?php   for($j = 0; $j < count($categorie); $j++) { 
            ...
            <tr>
				...
				<td><?php 	$sousforum = $bddConnection->prepare('SELECT * FROM cmw_forum_sous_forum WHERE id_categorie = :id_categorie');
							...
							{ ?>
						<div class="dropdown">
						<button class="btn btn-parabellum dropdown-toggle" type="button" id="sous-forum<?php echo $categorie[$j]['id']; ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" style="width: 99.5%;">
						...
						<ul class="dropdown-menu" aria-labelledby="sous-forum<?php echo $categorie[$j]['id']; ?>">
							<?php for($s = 0; $s < count($sousforum); $s++) {
								?><li><a class="text-muted" href="?&page=forum_categorie&id=<?php echo $categorie[$j]['id']; ?>&id_sous_forum=<?php echo $sousforum[$s]['id']; ?>"><?php echo $sousforum[$s]['nom']; ?></a></li>
							<?php } ?>
						</ul>
						<?php } ?>
						</div>
...
{
	?><a class="btn btn-parabellum btn-xs btn-block" role="button" data-toggle="collapse" href="#add_forum" aria-expanded="false" aria-controls="add_forum">
...
{
	?>
	<a class="btn btn-parabellum btn-xs btn-block" role="button" data-toggle="collapse" href="#add_categorie" aria-exepanded="false" aria-controls="add_categorie"> Ajouter un Forum</a>
	<div class="collapse" id="add_categorie">
			<form action="?action=create_cat" method="post"><br>
				...
				<div class="form-group row">
						...
						<div class="col-md-6">
							<select name="forum" class="form-control text-muted" require>
								<?php
								for($z = 0; $z < count($fofo); $z++)
								{
									?><option class=" text-muted" value="<?php echo $fofo[$z]['id']; ?>"><?php echo $fofo[$z]['nom']; ?></option><?php
								}
								?></select>
						</div><br/>
				</div>
